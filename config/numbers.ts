...
/**
 * hero 帧表映射（Q1-T03 三轮修正定版）：
 * 素材实物 = 77 §2.2 六帧表口径——00 待机/01 左脚在前(收步)/02 右脚在前(迈开)/03 左脚在前(收步变体)
 * /04 举手预备/05 挥出/06 普攻（01/02/03 腿位经拼图严格对比核验）。
 * ①一轮：04 出招预备混入 walk = 「抬剑帧」→ 移出；
 * ②二轮：walk=02/03 → 仍「滑」（两帧同为迈开姿势、开合变化不足，交替感弱）；
 * ③三轮（Leo：「要两只脚交替走的两帧」）：walk = 01↔02 左右脚严格交替（收步↔迈开，开合差最明显）。
 * 04~06 留给 T06 战斗出招。
 */
