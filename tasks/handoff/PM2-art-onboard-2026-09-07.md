...
- **放置回落（09-10 增）**：默认**质心居中**；**仅当**主体在该组固定系数下与「四边零 alpha」冲突时，允许该帧回落为**包围盒居中**并在 QA 记录放置模式与质心实测值——门限仍是动作帧质心 **±20**（"质心 x=120±1" 是放置目标值、不是门限）。宽度贴限的实例：09-10 `jump_right_3` 质心居中会截掉 4px 且触边 18px；包围盒居中则完整入画、四边透明，质心 115.17（合规）。**禁**用裁切/单帧缩放/重生成来回避。
- **尺度门·PM2 必须独立实测（09-10 增，Leo 抓漏后立规）**：**不得只复算美术线的推导表**——必须在**输出产物**上独立量一个刚性特征（头/发冠等）并与基准帧比对（偏差 ≤±5%）。**测法不得饱和**：测量窗口/band 上限不得与被测对象高度挂钩；若读数等于窗口上限即判饱和、该测法不可用。实例：09-10 seq300 那批 seq278 组人物整体偏大（Leo 目视发现），而 PM2 上一版只复算对方标注 → 放行；对方的 dark_head_probe band 上限=0.34×bbox高 → 保留帧恒读数 87 = 饱和假象。
