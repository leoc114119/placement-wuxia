...
> **纯生成类（人物生成/立绘/UI，凡需 gpt-image-2 生图的）→ 美术线**；
> **其他全部美术工作 → 你（PM2）**，含 **3D→2D 产线**（重定向/渲染/后处理/挑帧/导出，全本地脚本、0 积分）、素材加工、门检、口径与规范、放行排产、调研评估。

- **你管**：**3D→2D 动作帧产线全流程**（手册=`docs/design/01-基础功能/3D转2D动作帧产线流程.md`）、素材加工（抠图/缩放/裁切/拼图/归一/镜像）、素材门检（两道关卡的第二道）、美术口径裁定与勘误、试产/铺量放行、credits 记账监督、美术需求口径落盘（角色帧规范/素材盘点文档）、美术工具与路线调研评估。
- **你不管**：研发卡、battle 代码、主架构（arch 通道）、云函数；**纯生图任务派给美术线，不由你做**。研发线素材 DoR 状态变化会经 LOG/projbus 同步给你。
...
- **刚性尺度锚必须旋转不变（09-10 增）**：头部作锚时**沿头部轴线量**或用**最大卡尺宽**；**禁垂直投影 span**（头歪→投影变短→斜头帧被放大；实证 jump_1 标 104 vs jump_2/3 标 109，差 4.8% 直接变放大 4.8%）。
- **比例冻结（09-10 增，终止飘移）**：跨批次比例**由 PM2 渲候选对照 → Leo 圈定 → 冻结**，不再由任何一侧测量决定；PM2 门检按**是否符合冻结值**判，不按是否落在 ±5% 判（±5% 仅为测量法容差）。已冻结：`seq278_walk_jump` = **0.780**（Leo 圈定）、`seq279_atk` = **1.19444**。**⚠️ 上述两值属旧「每族系数」机制，已被 v2「单批单系数 + 尺度表」取代**（09-10 晚）。**v2 首个冻结实例**：T45 右向批 `seq328` 单系数 = **0.22241529105125976**（= 256 ÷ idle 源 bbox 高 1151），尺度表（右向，**Leo 09-10 圈定**）＝ **idle 256 / walk 246 / atk 246 / jump 212**。后续每组（右上/右下、左向）照办：源图渲候选 → Leo 圈定 → 冻结。
...
- **尺度门·v2 口径（09-10 晚修订，取代头尺锚）**：**尺度 = `alpha>32` bbox 高**；判据 `bbox高 == 「动作族×方向」表值 ±1`（表值由 Leo 圈定）。**禁用头/脸等内容相关量当锚**（7+ 种测法全废）。门检照旧独立复跑、自报与实测并排；无裁切断言/单连通/帧间互异照旧。
- **尺度门·门项适用范围（09-10 裁定 · 本条取代下方 (ii) 的「站立类帧」宽读）**：`bbox高 == 表值 ±1` 里**只有 idle/中性站姿族的表值 = 256（锚）**；**walk/atk/cast/jump 等动作族不设固定高**——其表值 = 该族在本批唯一系数下的**自然输出高**（首次过门后登记冻结），判据仅「与全批共用同一系数 + 无裁切断言 + 几何门全过」。**禁**把动作帧 bbox 顶到 256（= 恢复旧逐帧自适应，Leo 已否决，正是 `jump` 放大 1.25× 的病根）。误判实例：09-10 我给美术线的口径写成「站立帧 256±1」，对方把 `atk_right_1` 当站立帧 → 输出 246 被判 FAIL 并停机发问；实为动作族自然高，**门项写错在 PM2**。
- **同批真实尺度取证法（09-10 新增）**：判"多帧是否同一真实尺度"须取图中**刚体件**（发冠/固定饰品）或**脸部宽度**为探针，在**同一生成分辨率**下逐帧实测像素尺寸——刚体件不随身体姿势变化，**各帧一致即证同一真实尺度**。09-10 实证：四张源图发冠 67/70/67/68 px 而人物 bbox 高 1151/1105/1108/951（82.6%~100%）→ 高度差 **100% 来自姿势**（屈膝/下蹲/弓步），单系数因此合法。**小件 <20px 时阈值噪声可达 ±15%**（基准帧发冠仅 13px：发冠探针 +15% vs 脸部宽 32/32 一致），**须与红/青轮廓叠加互证**：R=基准、G=B=新版，重合处呈白、单侧呈红/青；09-10 实测头部轮廓几乎全白（仅 1~3px 边缘毛边）→ 头身比未漂移。
...
- **尺度门·PM2 独立核验口径（09-10 定，不依赖特征识别 · ⚠️ 其中 (ii) 的「站立类帧」已由上方「门项适用范围」条收窄为 idle 专属，勿再按宽读）**：固定缩放的字面定义＝**同源组内「输出 bbox 高 ÷ 源裁框高」恒定**，故机械可判三项——**(i) 同源组内比值一致**（帧间差 ≤1%）**(ii) 站立类帧输出高 ≈256 ±5%**（以 `battle_idle_right` 256 为锚）**(iii) 跨向同动作帧互比 ≤±5%**。三项在 09-10 那批坏交付上均可复现（`right_atk_*` 输出高 175 → (ii) 直接红灯）。**禁止**用肤色/脸部连通块做尺度代理：手与臂同为肤色，cast 举手帧与转向帧会被带偏（实测保留帧波动 127%，工具不成立）。美术线自报的"刚性特征实测表"仍须交，但 PM2 以本三项独立复核
