...
- **比例冻结（09-10 增，终止飘移）**：跨批次比例**由 PM2 渲候选对照 → Leo 圈定 → 冻结**，不再由任何一侧测量决定；PM2 门检按**是否符合冻结值**判，不按是否落在 ±5% 判（±5% 仅为测量法容差）。已冻结：`seq278_walk_jump` = **0.780**（Leo 圈定）、`seq279_atk` = **1.19444**。后续每组（右上/右下的源批次）照办。
