...
| 2026-09-08 | 主架构（Codex） | ✅ seq=135 main 追溯技术验收 PASS | 以 `main@92ba7de8edd7ff0f290f4d17f94b490b49feacee` 复核 v0.3 两段时序、v0.4 普攻交互与 seq=131 三处观感修正：typecheck/lint/build 通过；battle 322+14 skipped、behavior 14/14；shot_sixdir 48、shot_as_cast 三档 x1/x2、shot_basic_cells 36/36、e2e 11 MATCH；bundle `DBG[`=0。结论写入 `tasks/threads/ARCH-AS.md` 与方案 §9.6，关闭 seq=117/121/130/131/135 主架构等待项；tests 既有 3 处 eslint 报错非本链引入且不阻塞。 |
| 2026-09-08 | 主架构（Codex） → rd | 📤 projbus 验收交付 seq=140/141 | seq=140 发送 main 全链 PASS 与三份留痕路径；随后核对 commit 全 SHA 后以 seq=141 更正交付字段为 `a14e78fbc4503420ed09cc6d916aabcf523ed1f3`，该提交已 push `origin/main`，三路径均可核验。 |
| 2026-09-14 | Codex（arch）→ rd | 📐 T31 2.5D S0 技术前提实测方案完成（seq=404） | 定版 raw WebGL2、自研 41 骨 uniform 蒙皮、独立 proto/webgl2_probe；普通分包装 48k 模型副本；A1=离屏 webgl2/readPixels/drawImage 两安卓×三冷启动，A2=1/5/10/20 档并按 20 单位 30fps/P95 33.3ms 判；浏览器/开发者工具只作预验证，安卓真机唯一判定源；失败按主画布正控转 B/C。 |
| 2026-09-14 | ZCode（frontend） | 📦 T31-FE-B 交付 + 提问（task/t31-fea @ 5ad21b46，已 push 同分支） | 主角 3D 战斗接线与预览（方案 §8 卡 B，基座 5ec1ec1a）：接点（§4.1 单一坐标出口 / §4.3 槽位合成一次 / placed 与 HUD 热区同源 / 3D 无 2D 双画 / 未就绪不切 2D 帧）+ 宿主装配（loader 资源门失败停在失败页给重试）+ 2D 武器层对 3D hero 停用（配置素材保留）。自验：typecheck/lint/build 0 错、battle 674+14 skipped（基线 654/14）、behavior 14/14、shot 16 PASS、e2e 11/11 MATCH、shot_character3d 44/44（65 张证据，脚底误差 ≤1.78 物理像素）。提问 Q1-T31-FEB（轻功动作窗被 isJump 截断，待裁）/ Q2-T31-FEB（参考屏高像素基准、renderer 首调早退、分支文档落后真源 2 行）。 |
