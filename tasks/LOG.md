...
| 2026-09-13 | Codex（arch）→ rd | 📐 T30 hero 六向 5 帧实装方案完成（seq=395） | 五 clip 全 5 帧、die=1；idle 140ms 循环；walk 取 run 且 140ms；charge cast1→5 循环、strike cast4→5 单播、basic atk1→5；jump 按 ma.t/ma.duration 尾帧保持并 hop=0；F1 武器入口注释留 TODO、代码资产保留；ver t45v3；可发 frontend 单卡。 |
