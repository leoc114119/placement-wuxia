...
| 09-04 | Leo | 📋 L环反馈 | 六向朝向视觉层测不了——现战斗无方向帧（旧 T06 帧仅左右翻转），视觉验收天然依赖 T45 素材 |
| 09-04 | ZCode | 📋 验收拆层定版 | FACE-1 验收拆两层：数据层（hexFacing 正确性）=主架构推演+6 用例+SP-2 锁，已验过；视觉层（六向上屏）挂账并入 T45 接线卡 L 环一次验（朝向+人物）。卡状态=数据层验收通过，不完整闭环不打回 |
