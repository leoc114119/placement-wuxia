...
## Q3-T03-R2 补充节 · UI 自适应布局（三段式锚定，ZCode 08-22 02:40 交付，待 C 环）

> 工单：`tasks/questions/Q3-T03.md`（R2 版，取代 01:15 绝对坐标方案；T03 主卡已归档，回执补充节落此文件）

### DoD 勾选

- [x] typecheck / lint / build 三零；`test:battle` **30/30**（27 既有不回归 + R2 新增 3）
- [x] node 模拟两档分辨率（375×667 / 375×812）：SCENE_RECT 边界、标签/按钮随锚点移动、绘制区域=SCENE_RECT（ctx 录制断言背景 drawImage = 窗口内 cover 居中裁切）
- [x] GUI 截图复核：真代码 preview 页（加载 dist 产物 + wx shim）两档各一张 + 点击移动验证（`preview/shots/q3r2_375x667.png` / `q3r2_375x812.png` / `q3r2_375x812_clickmove.png`）
- [x] threads/LOG 双写登记（见下）

### 实现要点

1. **布局唯一真源** `computeSceneLayout(size, statusBarBottomPx?)`（systems/scene.ts）：状态栏底（胶囊优先，fallback `STATUS_FALLBACK_RATIO=0.085`）→ Tab 顶（`TAB_BAR_H_RATIO=0.08` 锚底）→ SCENE_RECT（两界之间全宽）→ 标签中心（状态栏底+0.03H）/ 按钮行中心（窗底−r−0.02H）。渲染与 hitSceneButton 共用，命中优先逻辑零改动。
2. **胶囊取值** `getStatusBarBottomPx(canvasW)`：`wx.getMenuButtonBoundingClientRect().bottom + 8px` 按 dpr 换算物理 px，一次性取值；异常/无胶囊 → 0 走 fallback；异常大防御性夹半屏。
3. **背景改 SCENE_RECT cover**：clip 矩形裁切 + 居中 cover（替代全屏铺满与 BG_FIT_CONTAIN，常量已删）；全屏宣纸清屏保留（预留区同色）。
4. **绝对比例常量清除**：SCENE_LABEL.y、SCENE_BUTTONS.yRatio、BG_FIT_CONTAIN 删除，全部换锚定推导。
5. **按钮图标接线**：`SCENE_BUTTON_DEFS` 改 iconSrc（btn_biguan/guaji/tiaozhan.png 1280²），预载进 `SceneAssets.buttonIcons`，drawImage 绘制（缺图跳过图标只画文字）；文字标签保留代码绘制。
6. **WALK_ZONE 屏幕坐标未动**；移动语义、hitSceneButton 优先级未动。

### 改动文件

| 文件 | 变更 |
|---|---|
| `config/numbers.ts` | 三段式锚定常量（MENU_BOTTOM_MARGIN_PX/STATUS_FALLBACK_RATIO/TAB_BAR_H_RATIO/SCENE_LABEL_OFFSET_RATIO/BUTTON_ROW_GAP_RATIO）；SCENE_LABEL/SCENE_BUTTONS 去绝对坐标；SCENE_BUTTON_DEFS iconSrc；删 BG_FIT_CONTAIN |
| `systems/scene.ts` | computeSceneLayout/SceneLayout/getStatusBarBottomPx/ViewSize.statusBarBottomPx；layoutSceneButtons 改锚定 |
| `ui/render.ts` | render 第三参 statusBarBottomPx；SCENE_RECT cover 背景；标签/按钮锚定绘制；图标 drawImage |
| `ui/assets.ts` | 预载按钮图标 ×3 |
| `game.ts` | 启动取胶囊下沿，注入 bindTapInput 与 render |
| `env.d.ts` | wx.getMenuButtonBoundingClientRect + WxMenuButtonRect |
| `types.ts` | SceneButton.iconSrc；SceneAssets.buttonIcons |
| `tests/scene.test.ts` | 布局用例重写为锚定口径 + 两档分辨率断言 + ctx 录制绘制区域断言（+3 用例） |
| `preview/game-preview.html` | **新增**：真代码渲染预览页（CommonJS 微加载器跑 dist 产物 + wx shim + 锚定参考线诊断） |
| `preview/shots/q3r2_*.png` | **新增**：两档截图 + 点击移动验证图 ×3 |
