...
## 9. L 环终验两条受理链查修（查修一体 · 真根因报告）

> PM 模式修正：排查方向仅为初步猜测，以实际定位为准。以下为复现实证后的**真根因**。

### ① 特/绝选中后无法释放 → session 受理链零缺陷（真根因在 FE 命中层，已移交）
- **node 链路复现（全通）**：激活 te（true，attackCells 6 格、敌格 ∈ 集合）→ submit(attack te) → true + `skill:te` 事件 + bar 100→0。Q1 取消降级后的分流（激活态点敌=施放选中技/无选中点敌=普攻）路径完整无遗漏。
- **真根因定位（FE 命中层）**：`ui/battle-input.ts` L127 敌棋子命中按 **renderPos（lerp 动画位）** 匹配——敌移动动画中 renderPos≠逻辑 hex，点击视觉敌格命中落空 → 走 L151「skill && !inAttack → cancelSkill」把选中取消 → 后续点击无红格无响应（Leo 观感「无法释放」的完整链条：一次错位点击=取消选中）。
- **处置**：session 零改动（受理链无缺陷）；命中错位修法（按逻辑 hex 命中敌棋子）归 FE 演出层修复（与其长距闪烁修复同批）。已加锁死用例：[ATK-2] 合法结算（激活→点射程内敌→skill 事件+内力-1+条清零）与四查拒绝路径已在库。

### ③ 轻功跳一次后穿模 → session 语义零缺陷（真根因=渲染直线 lerp 表达绕行移动，已移交 FE）
- **逐环实证**：跳一次（selection 清除）→ 输入态恢复 → **moveCells ≡ 同参 reachable 恒等**（57 格恰为普通可达，无跳跃快取残留混入）→ 敌占格恒排除 → jump-only 格（若有）提交必拒。
- **真根因定位（FE 表达层）**：「穿越」观感 = **绕行合法移动的直线 lerp 动画**——BFS 判定不可穿（语义正确），但 FE 棋子动画是起点→终点直线插值，绕行路径被画成穿过中间单位。跳一次后移动力 6 的玩家绕行格多，观感频次升高。
- **处置**：session 零改动（语义正确）；修法归 FE 演出层（绕行路径分段的移动动画/或 lerp 遮罩）。已加锁死用例：[L 环终验③] 跳后 moveCells≡walk BFS 恒等 + jump-only 格提交拒绝 + 敌占格排除（快取残留/旧快取消费必挂）。

### DoD 终态
- 三零：typecheck 0 / build 0 / lint 0（唯一 lint error 在 FE 在途文件 battle-hex-render.ts 'JUMP' unused——FE 跳跃插值实现中，归属 FE 收尾）
- 全量：**144/144**（含 FE 并行新增用例；期间两次 render/dbg_frames 瞬态失败为 FE 编辑器实时保存半成品，复跑自愈——已用 stash/精确隔离两轮验证归属）
- 本侧改动不变（session 重写+input D4 三行+测试）；规格 sticky 无需勘误（v1.0 本义即无连放，前轮已收编）。
