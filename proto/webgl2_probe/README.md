...
| 14 | HONOR 真机归档件里 `a1.errors` 含「41 骨 palette 上传自证失败」，与**同机 `DEVICE_PASS`** 自相矛盾 | 该引擎 `getUniformLocation` 能定位、但 `getUniform` **读回 null**；分类器把"located 但读不到值"当成了失败 | 分类口径修正：**拿不到值（null/空数组/未定位）= 平台不回读 = `unsupported`**（不进 errors、不影响判定）；只有"确实读回了矩阵、值与上传不符"才算真失败（那才是这条自证要抓的）。套件加**返回 null 形态**回归门（模拟真机：断言 unsupported=true、errors 为空、判定不误报 FAIL） |
...
命令：`node proto/webgl2_probe/tests/probe-browser.mjs --runs=3 --profile=spec` → **97/97 PASS**（主检查 + 真机判定矩阵回归门 + 结果回收四兜底交互 + 回读降级模拟（抛错/返回 null 两态）+ 触摸坐标空间回归门；约 3.5 分钟）。
