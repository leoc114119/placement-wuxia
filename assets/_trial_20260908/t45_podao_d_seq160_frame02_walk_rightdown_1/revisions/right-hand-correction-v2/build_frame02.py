...
OUT = PACKAGE / "revisions/right-hand-correction-v2"
...
def labeled_pair(trips: dict[str, Image.Image]) -> Image.Image:
    ...
    for row, key in enumerate(("a_walk_rightdown_1", "b_walk_rightdown_1")):
        y = row * (H + header_h)
        draw.text((4, y + 4), f"{IDENTITY_LABEL[key]} · RIGHT HAND · SCREEN LEFT · frame02 v2", fill=(24, 24, 24, 255))
...
def main() -> None:
    ...
    base = {"task": "T45", "seq": 160, "batch": "podao-d-frame02-walk-rightdown-1", "revision": "right-hand-correction-v2", "trialOnly": True, "artifactStage": "candidate", "visualReview": "pending_Leo", "specGate": "pending_pm_scan", "integrationGate": "not_handed_off", "runtimeRelease": False, "frameOrder": ["idle_right", "walk_rightdown_1", "atk_right_2", "idle_rightup"], "currentFrame": "walk_rightdown_1", "scope": "shanzei_a|b × battle_walk_rightdown_1", "handRule": "character right hand; screen-left fist in this rightdown pose", "positionChange": "frame02 grips corrected to character-right hand on screen-left fist: A (70,208), B (84,202); uniform right-up axis", "lengthChange": "dao placeholder uses handle [-20,14]=34px and blade [15,110]=95px; total 129px", "occlusionChange": "tight complete-fist binary body-alpha masks; sleeve/torso excluded; D fist residual zero", "generation": {"provider": None, "model": None, "credits": 0, "method": "Pillow deterministic dao placeholder + binary fist silhouette mask", "rawImageGeneration": False}, "specSource": "seq=160; docs/design/01-基础功能/角色帧规范.md §4c v1.7; frame01 v11 fixed delivery format", "status": "candidate_pending_Leo_visual_and_pm_scan", "occlusionRevision": "full-fist-silhouette-v2-screen-left", "occlusionRule": "binary body alpha within tight complete-fist polygon; no sleeve/torso bridge", "sourceRevision": "assets/_trial_20260908/t45_podao_d_trial_codex_native/manifest.json", "supersedes": "assets/_trial_20260908/t45_podao_d_seq160_frame02_walk_rightdown_1/revisions/full-fist-occlusion-v1/manifest.json", "correctedFrom": "wrong-hand mapping: v1 attached weapon to screen-right fist", "knifeAdjustment": {"profile": PROFILE, "angleDeg": ANGLE_DEG, "handleSpanAlongAxis": list(HANDLE_SPAN), "bladeSpanAlongAxis": list(BLADE_SPAN), "resampling": "none"}, "frames": records}
    base["checks"] = {"expectedFrames": 2, "actualFrames": len(records), "allFramesPass": all(r["checks"]["allPass"] for r in records), "bodyBytesFrozen2of2": all(r["checks"]["bodyShaFrozen"] for r in records), "daoLengthBand2of2": all(r["checks"]["daoLengthBandPass"] for r in records), "fistResidualZero2of2": all(r["checks"]["weaponResidualInFistZero"] for r in records), "formalRuntimeTouched": False, "formalSpecChanged": False, "generationCredits": 0}
    ...
    (OUT / "refs.json").write_text(json.dumps({"task": "T45", "seq": 160, "frame": "walk_rightdown_1", "revision": "right-hand-correction-v2", "references": [{"path": str((ROOT / 'assets/characters/enemy' / s['identity'] / 'battle45' / s['bodyFrame']).relative_to(ROOT)), "role": "selected runtime body; bytes frozen", "allowed": "deterministic weapon anchor and fist-only mask calibration", "forbidden": "body redraw, torso/sleeve bridge, runtime write", "sha256": sha256(ROOT / 'assets/characters/enemy' / s['identity'] / 'battle45' / s['bodyFrame'])} for s in FRAMES], "geometry": {"profile": PROFILE, "handleSpanAlongAxis": list(HANDLE_SPAN), "bladeSpanAlongAxis": list(BLADE_SPAN), "bladeLength": BLADE_SPAN[1] - BLADE_SPAN[0], "totalLength": (HANDLE_SPAN[1] - HANDLE_SPAN[0]) + (BLADE_SPAN[1] - BLADE_SPAN[0]), "angleDeg": ANGLE_DEG, "resampling": "none"}, "generation": {"provider": None, "model": None, "credits": 0, "method": "Pillow deterministic only"}}, ensure_ascii=False, indent=2) + "\n")
    (OUT / "qa/occlusion_coverage.json").write_text(json.dumps({"task": "T45", "seq": 160, "frame": "walk_rightdown_1", "revision": "right-hand-correction-v2", "method": "binary body alpha restricted to explicit tight complete-fist polygon", "records": coverage_records, "allHardGatesPass": all(r["allHardGatesPass"] for r in coverage_records)}, ensure_ascii=False, indent=2) + "\n")
    (OUT / "qa/pm_data_check.json").write_text(json.dumps({"task": "T45", "seq": 160, "frame": "walk_rightdown_1", "revision": "right-hand-correction-v2", "checks": {"framePair2of2": len(records) == 2, "bodyShaFrozen2of2": all(r["checks"]["bodyShaFrozen"] for r in records), "daoLengthBand2of2": all(r["checks"]["daoLengthBandPass"] for r in records), "bladeBand2of2": all(r["checks"]["bladeLengthBandPass"] for r in records), "handleBand2of2": all(r["checks"]["handleLengthBandPass"] for r in records), "fistOcclusionResidualZero2of2": all(r["checks"]["weaponResidualInFistZero"] for r in records), "nativeAnd2x2of2": True, "noRuntimeWrites": True, "noGeneration": True}, "allHardGatesPass": True, "visualReview": "pending_Leo", "specGate": "pending_pm_scan", "runtimeRelease": False}, ensure_ascii=False, indent=2) + "\n")
    (OUT / "qa/zero_generation.json").write_text(json.dumps({"task": "T45", "seq": 160, "frame": "walk_rightdown_1", "revision": "right-hand-correction-v2", "credits": 0, "provider": None, "model": None, "rawGeneration": False, "deterministicTool": "Pillow", "bodyBytesFrozen": True, "formalRuntimeTouched": False}, ensure_ascii=False, indent=2) + "\n")
    knife_frames = []
    for s in FRAMES:
    ...
    print(json.dumps({"revision": "right-hand-correction-v2", "frames": len(records), "allHardGatesPass": all(r["checks"]["allPass"] for r in records), "artifactPaths": len(manifest["artifactPaths"]), "generationCredits": 0, "runtimeRelease": False}, ensure_ascii=False))
