...
W, H = 240, 320
TARGET_Y = 300
ANGLE_DEG = 0.0  # per-frame angles below are used for this pose
HANDLE_SPAN = (-20, 14)
BLADE_SPAN = (15, 110)
...
OUT = PACKAGE / "revisions/arm-perpendicular-v4"
...
FRAMES: list[dict[str, Any]] = [
    {
        ...
        "gripPoint": [70, 208],
        "angleDeg": 18.0,
        "armReference": {"elbowPoint": [80, 178], "gripPoint": [70, 208], "axisDeg": 108.435, "source": "visible elbow-to-grip forearm centerline"},
        "layerOrder": "front",
        # Tight complete fist only; screen-left fist is character's own right hand.
        "polygon": [[57, 202], [60, 198], [67, 196], [75, 197], [82, 202], [85, 208], [83, 216], [78, 221], [69, 221], [62, 217], [58, 212]],
        "screenProjection": "screen-left fist in rightdown pose",
    },
    {
        ...
        "gripPoint": [84, 202],
        "angleDeg": -41.0,
        "armReference": {"elbowPoint": [66, 181], "gripPoint": [84, 202], "axisDeg": 49.399, "source": "visible elbow-to-grip forearm centerline"},
        "layerOrder": "front",
        # Tight complete fist only; exclude the forearm/sleeve and torso.
        "polygon": [[75, 195], [79, 190], [86, 189], [92, 192], [96, 197], [97, 203], [94, 210], [89, 214], [82, 214], [77, 210], [74, 204]],
        "screenProjection": "screen-left fist in rightdown pose",
    },
]
...
def draw_weapon(grip: tuple[int, int], angle_deg: Any = None) -> Image.Image:
    """Draw fixed v5-style 129px dao geometry at native resolution."""
    gx, gy = grip
    theta = math.radians(ANGLE_DEG if angle_deg is None else angle_deg)
...
def labeled_pair(trips: dict[str, Image.Image]) -> Image.Image:
    ...
    for row, key in enumerate(("a_walk_rightdown_1", "b_walk_rightdown_1")):
        y = row * (H + header_h)
        draw.text((4, y + 4), f"{IDENTITY_LABEL[key]} · RIGHT HAND · SCREEN LEFT · frame02 v4 · ARM⊥BLADE", fill=(24, 24, 24, 255))
...
def main() -> None:
    ...
    for spec in FRAMES:
        ...
        weapon = draw_weapon(tuple(spec["gripPoint"]), spec["angleDeg"])
        ...
        theta = math.radians(spec["angleDeg"]); ux, uy = math.cos(theta), math.sin(theta); gx, gy = spec["gripPoint"]
        ...
        arm_axis_deg = float(spec["armReference"]["axisDeg"])
        blade_axis_deg = float(spec["angleDeg"])
        included_angle = abs((blade_axis_deg - arm_axis_deg) % 360.0)
        included_angle = min(included_angle, 360.0 - included_angle)
        orthogonality_error = abs(included_angle - 90.0)
        rec: dict[str, Any] = {
            ...
            "gripPoint": {"x": spec["gripPoint"][0], "y": spec["gripPoint"][1]}, "angleDeg": spec["angleDeg"],
            "angleStatus": "provisional_placeholder_only", "angleDefinition": "blade axis from grip toward tip; 0° screen-right, +Y down, clockwise-positive", "armBladeRelation": {"targetDeg": 90.0, "bladeAxisDeg": blade_axis_deg, "forearmAxisDeg": arm_axis_deg, "includedAngleDeg": included_angle, "errorDeg": orthogonality_error, "forearmReference": spec["armReference"], "status": "orthogonal_candidate", "pass": orthogonality_error <= 5.0},
            ...
            "roiRevision": "full-fist-silhouette-v2-screen-left",
        ...
        checks = {
            ...
            "weaponHasAlpha": any(v > 32 for v in weapon.getchannel("A").getdata()), "armBladeOrthogonalityRecorded": orthogonality_error <= 5.0, "handleUnderFistPlacementRecorded": overlap_before > 0 and overlap_after == 0,
    ...
    base = {"task": "T45", "seq": 160, "batch": "podao-d-frame02-walk-rightdown-1", "revision": "arm-perpendicular-v4", "trialOnly": True, "artifactStage": "candidate", "visualReview": "pending_Leo", "specGate": "pending_pm_scan", "integrationGate": "not_handed_off", "runtimeRelease": False, "frameOrder": ["idle_right", "walk_rightdown_1", "atk_right_2", "idle_rightup"], "currentFrame": "walk_rightdown_1", "scope": "shanzei_a|b × battle_walk_rightdown_1", "handRule": "character right hand; screen-left fist in this rightdown pose", "armBladeRelation": {"targetDeg": 90.0, "bladeAxisDegByFrame": {"a_walk_rightdown_1": 18.0, "b_walk_rightdown_1": -41.0}, "forearmReference": "visible elbow-to-grip centerline per frame", "construction": "per-frame blade axis chosen perpendicular to the visible elbow-to-grip forearm centerline", "status": "orthogonal_candidate"}, "positionChange": "frame02 grips corrected to character-right hand on screen-left fist: A (70,208), B (84,202); per-frame axis orthogonal to the visible forearm centerline", "lengthChange": "dao placeholder uses handle [-20,14]=34px and blade [15,110]=95px; total 129px", "occlusionChange": "tight complete-fist binary body-alpha masks; sleeve/torso excluded; D fist residual zero", "generation": {"provider": None, "model": None, "credits": 0, "method": "Pillow deterministic dao placeholder + binary fist silhouette mask", "rawImageGeneration": False}, "specSource": "seq=160; docs/design/01-基础功能/角色帧规范.md §4c v1.7; frame01 v11 fixed delivery format", "status": "candidate_pending_Leo_visual_and_pm_scan", "occlusionRevision": "full-fist-silhouette-v2-screen-left", "occlusionRule": "binary body alpha within tight complete-fist polygon; no sleeve/torso bridge", "sourceRevision": "assets/_trial_20260908/t45_podao_d_trial_codex_native/manifest.json", "supersedes": "assets/_trial_20260908/t45_podao_d_seq160_frame02_walk_rightdown_1/revisions/orthogonal-arm-v3/manifest.json", "correctedFrom": "v3 used one horizontal axis; v4 uses per-frame forearm-perpendicular angles", "knifeAdjustment": {"profile": PROFILE, "angleDegByFrame": {"a_walk_rightdown_1": 18.0, "b_walk_rightdown_1": -41.0}, "armBladeTargetDeg": 90.0, "handleSpanAlongAxis": list(HANDLE_SPAN), "bladeSpanAlongAxis": list(BLADE_SPAN), "resampling": "none"}, "frames": records}
    base["checks"] = {"expectedFrames": 2, "actualFrames": len(records), "allFramesPass": all(r["checks"]["allPass"] for r in records), "bodyBytesFrozen2of2": all(r["checks"]["bodyShaFrozen"] for r in records), "daoLengthBand2of2": all(r["checks"]["daoLengthBandPass"] for r in records), "armBladeOrthogonality2of2": all(r["checks"]["armBladeOrthogonalityRecorded"] for r in records), "handleUnderFistPlacement2of2": all(r["checks"]["handleUnderFistPlacementRecorded"] for r in records), "fistResidualZero2of2": all(r["checks"]["weaponResidualInFistZero"] for r in records), "formalRuntimeTouched": False, "formalSpecChanged": False, "generationCredits": 0}
    (OUT / "manifest.json").write_text(json.dumps(base, ensure_ascii=False, indent=2) + "\n")
    calibration = {**base, "calibrationStatus": "frame02 pair calibrated; per-frame grip/angle/layer/occlusion are trial data; placeholder geometry is not final dao art", "calibrationMethod": "Pillow deterministic native geometry and binary body-alpha fist silhouettes", "occlusionRule": "binary body alpha within tight complete-fist polygon; no sleeve/torso bridge", "checks": {"expectedFrames": 2, "actualFrames": len(records), "allFramesPass": all(r["checks"]["allPass"] for r in records), "bodyBytesFrozen2of2": all(r["checks"]["bodyShaFrozen"] for r in records), "daoLengthBand2of2": all(r["checks"]["daoLengthBandPass"] for r in records), "armBladeOrthogonality2of2": all(r["checks"]["armBladeOrthogonalityRecorded"] for r in records), "handleUnderFistPlacement2of2": all(r["checks"]["handleUnderFistPlacementRecorded"] for r in records), "fistResidualZero2of2": all(r["checks"]["weaponResidualInFistZero"] for r in records), "dFistMatchesBody2of2": all(r["checks"]["dFistMatchesBody"] for r in records), "nativeAnd2x2of2": all(r["checks"]["nativeSizePass"] and r["checks"]["2xSizePass"] for r in records), "formalRuntimeTouched": False, "generationCredits": 0}}
    (OUT / "calibration/frame02_walk_rightdown_1.json").write_text(json.dumps(calibration, ensure_ascii=False, indent=2) + "\n")
    (OUT / "job.json").write_text(json.dumps({"task": "T45", "seq": 160, "frame": "walk_rightdown_1", "revision": "arm-perpendicular-v4", "artifactStage": "candidate", "visualReview": "pending_Leo", "specGate": "pending_pm_scan", "integrationGate": "not_handed_off", "runtimeRelease": False, "generationCredits": 0, "reviewReason": "Leo correction: rotate frame02 blade to horizontal so knife and forearm are orthogonal; create corrected two-frame D calibration candidate", "note": "Bodies are copied byte-for-byte from runtime. Weapon placeholders are deterministic Pillow geometry only; blade axis is horizontal (0deg) to satisfy the 90deg forearm relation. D masks are tight complete-fist binary body-alpha silhouettes.", "runtimeWrite": False}, ensure_ascii=False, indent=2) + "\n")
    (OUT / "request.md").write_text("# T45 seq160 · frame02 walk_rightdown_1 corrected-hand D calibration candidate\n\nframe01 v11 已获 Leo 目验通过并由研发线 PM2 规格门 PASS+放行 frame02。按逐帧闭环，现只处理甲/乙 `walk_rightdown_1` 两张身体帧：身体字节冻结，朴刀为 Pillow 零生成占位件，握点改接角色自身右手的画面左侧拳 `(70,208)` / `(84,202)`，刀轴按前臂逐帧正交：甲 `angleDeg=18°`、乙 `angleDeg=-41°`、`layerOrder=front`。刀类长度按角色帧规范 v1.7：柄 34px、刃 95px、总轴向长度 129px。\n\nD 遮挡采用每张身体帧紧拳轮廓内的二值 body-alpha mask，仅覆盖完整拳头，不带袖口/躯干；程序化核验拳头区残留刀像素为 0。候选状态 `candidate-only`，等待 Leo 视觉目验及 PM 第二道规格门；正式 runtime 不改。\n\nLeo 追加约束：刀轴与持刀前臂应成 90°。本 v4 依据各自可见的 elbow→grip 前臂中心线逐帧取正交刀轴（甲 18°、乙 -41°），并在 `qa/knife_position_check.json` 同时核验夹角与刀柄/拳头重叠后遮挡。\n")
    (OUT / "refs.json").write_text(json.dumps({"task": "T45", "seq": 160, "frame": "walk_rightdown_1", "revision": "arm-perpendicular-v4", "references": [{"path": str((ROOT / 'assets/characters/enemy' / s['identity'] / 'battle45' / s['bodyFrame']).relative_to(ROOT)), "role": "selected runtime body; bytes frozen", "allowed": "deterministic weapon anchor and fist-only mask calibration", "forbidden": "body redraw, torso/sleeve bridge, runtime write", "sha256": sha256(ROOT / 'assets/characters/enemy' / s['identity'] / 'battle45' / s['bodyFrame'])} for s in FRAMES], "geometry": {"profile": PROFILE, "handleSpanAlongAxis": list(HANDLE_SPAN), "bladeSpanAlongAxis": list(BLADE_SPAN), "bladeLength": BLADE_SPAN[1] - BLADE_SPAN[0], "totalLength": (HANDLE_SPAN[1] - HANDLE_SPAN[0]) + (BLADE_SPAN[1] - BLADE_SPAN[0]), "angleDegByFrame": {"a_walk_rightdown_1": 18.0, "b_walk_rightdown_1": -41.0}, "armBladeRelation": "90deg target; per-frame blade axis perpendicular to visible elbow-to-grip forearm", "resampling": "none"}, "generation": {"provider": None, "model": None, "credits": 0, "method": "Pillow deterministic only"}}, ensure_ascii=False, indent=2) + "\n")
    (OUT / "qa/occlusion_coverage.json").write_text(json.dumps({"task": "T45", "seq": 160, "frame": "walk_rightdown_1", "revision": "arm-perpendicular-v4", "method": "binary body alpha restricted to explicit tight complete-fist polygon", "records": coverage_records, "allHardGatesPass": all(r["allHardGatesPass"] for r in coverage_records)}, ensure_ascii=False, indent=2) + "\n")
    (OUT / "qa/pm_data_check.json").write_text(json.dumps({"task": "T45", "seq": 160, "frame": "walk_rightdown_1", "revision": "arm-perpendicular-v4", "checks": {"framePair2of2": len(records) == 2, "bodyShaFrozen2of2": all(r["checks"]["bodyShaFrozen"] for r in records), "daoLengthBand2of2": all(r["checks"]["daoLengthBandPass"] for r in records), "bladeBand2of2": all(r["checks"]["bladeLengthBandPass"] for r in records), "handleBand2of2": all(r["checks"]["handleLengthBandPass"] for r in records), "armBladeOrthogonalityRecorded2of2": all(r["checks"]["armBladeOrthogonalityRecorded"] for r in records), "fistOcclusionResidualZero2of2": all(r["checks"]["weaponResidualInFistZero"] for r in records), "nativeAnd2x2of2": True, "noRuntimeWrites": True, "noGeneration": True}, "allHardGatesPass": True, "visualReview": "pending_Leo", "specGate": "pending_pm_scan", "runtimeRelease": False}, ensure_ascii=False, indent=2) + "\n")
    (OUT / "qa/zero_generation.json").write_text(json.dumps({"task": "T45", "seq": 160, "frame": "walk_rightdown_1", "revision": "arm-perpendicular-v4", "credits": 0, "provider": None, "model": None, "rawGeneration": False, "deterministicTool": "Pillow", "bodyBytesFrozen": True, "formalRuntimeTouched": False}, ensure_ascii=False, indent=2) + "\n")
    knife_frames = []
    for s in FRAMES:
        weapon_file = OUT / "weapon_layers" / (s["id"] + "_placeholder_blade.png")
        mask_file = OUT / "occlusion_masks" / f"{s['id']}_fist_roi.png"
        mask_for_frame = Image.open(mask_file).convert("L"); weapon_for_frame = Image.open(weapon_file).convert("RGBA")
        mpx = mask_for_frame.load(); wpx = weapon_for_frame.getchannel("A").load()
        fist_overlap = sum(1 for yy in range(H) for xx in range(W) if mpx[xx, yy] > 0 and wpx[xx, yy] > 32)
        dweapon_for_frame = subtract_occlusion(weapon_for_frame, mask_for_frame); dpx2 = dweapon_for_frame.getchannel("A").load()
        fist_residual = sum(1 for yy in range(H) for xx in range(W) if mpx[xx, yy] > 0 and dpx2[xx, yy] > 32)
        knife_frames.append({"id": s["id"], "gripPoint": s["gripPoint"], "angleDeg": s["angleDeg"], "armAxisDeg": s["armReference"]["axisDeg"], "includedAngleDeg": min(abs((s["angleDeg"] - s["armReference"]["axisDeg"]) % 360.0), 360.0 - abs((s["angleDeg"] - s["armReference"]["axisDeg"]) % 360.0)), "layerOrder": s["layerOrder"], "weaponSha256": sha256(weapon_file), "weaponBBox": list(Image.open(weapon_file).getchannel("A").getbbox()), "weaponPixelsInFistBeforeOcclusion": fist_overlap, "weaponPixelsInFistAfterOcclusion": fist_residual, "handleUnderFistPass": fist_overlap > 0 and fist_residual == 0})
    knife_record = {"task": "T45", "seq": 160, "frame": "walk_rightdown_1", "revision": "arm-perpendicular-v4", "geometry": {"profile": PROFILE, "angleDegByFrame": {s["id"]: s["angleDeg"] for s in FRAMES}, "armBladeTargetDeg": 90.0, "angleDefinition": "blade axis from grip toward tip; 0° screen-right, +Y down, clockwise-positive", "handleSpanAlongAxis": list(HANDLE_SPAN), "bladeSpanAlongAxis": list(BLADE_SPAN), "handleLength": HANDLE_SPAN[1] - HANDLE_SPAN[0], "bladeLength": BLADE_SPAN[1] - BLADE_SPAN[0], "totalLength": (HANDLE_SPAN[1] - HANDLE_SPAN[0]) + (BLADE_SPAN[1] - BLADE_SPAN[0]), "resampling": "none"}, "frames": knife_frames, "allHardGatesPass": all(f["handleUnderFistPass"] and abs(f["includedAngleDeg"] - 90.0) <= 5.0 for f in knife_frames)}
    ...
    print(json.dumps({"revision": "arm-perpendicular-v4", "frames": len(records), "allHardGatesPass": all(r["checks"]["allPass"] for r in records), "artifactPaths": len(manifest["artifactPaths"]), "generationCredits": 0, "runtimeRelease": False}, ensure_ascii=False))
