#!/usr/bin/env python3
"""Deterministic fist-center lock correction of T45 hero weapon frame01 sword grip.

The v6 normalized sword layer is reused without redraw. This revision shifts the whole layer
11px to screen-right so the handle contact lands at the frozen fist center (102,206); the
-40° axis, v5 upward placement and all body pixels are preserved.
...
REV = BASE / "revisions/fist-center-lock-v7"
BODY_PATH = ROOT / "assets/characters/hero/battle45/battle_idle_right.png"
SOURCE_LAYER = BASE / "revisions/right-up-placement-v6/normalized/hero_sword_held_rightup_v6.png"
SOURCE_RAW = BASE / "revisions/right-up-placement-v6/raw/hero_sword_held_transparent_source_attempt1.png"
OUT_LAYER = REV / "normalized/hero_sword_held_rightup_v7.png"
MASK_PATH = REV / "occlusion_masks/hero_idle_right_character_right_fist.png"
TRIP_PATH = REV / "composites_native/hero_idle_right_sword_triptych_v7.png"
TRIP_2X_PATH = REV / "composites_2x/hero_idle_right_sword_triptych_v7_2x.png"
CONTACT_PATH = REV / "contact/hero_idle_right_sword_pilot_v7.png"
CONTACT_2X_PATH = REV / "contact/hero_idle_right_sword_pilot_v7_2x.png"
QA_PATH = REV / "qa/pilot_v7.json"
CAL_PATH = REV / "calibration/frame01_idle_right_v7.json"
...
SOURCE_GRIP = (110.0, 185.0)  # v6 blade-base grip, character right hand; screen-left fist
...
RIGHT_SHIFT_PX = 11
TARGET_GRIP = (SOURCE_GRIP[0] + RIGHT_SHIFT_PX, SOURCE_GRIP[1])
SOURCE_ANGLE_DEG = -40.0
TARGET_ANGLE_DEG = -40.0
# Pixel lock: body skin-core centroid rounds to (102,206); v6 handle contact pixel is (91,206).
FIST_CENTER = (102.0, 206.0)
HANDLE_CONTACT_SOURCE = (91.0, 206.0)
HANDLE_CONTACT_TARGET = (HANDLE_CONTACT_SOURCE[0] + RIGHT_SHIFT_PX, HANDLE_CONTACT_SOURCE[1])
ROTATION_DELTA_DEG = 0.0
TRANSLATION_PX = (RIGHT_SHIFT_PX, 0)
...
def handle_pixel_pass(im: Image.Image, point: tuple[float, float]) -> bool:
    """Check that the locked point contains opaque brown/gold sword-handle pixels."""
    x, y = int(point[0]), int(point[1])
    r, g, b, a = im.getpixel((x, y))
    return a > 32 and ((r > 60 and r > g * 1.15 and g > b * 1.1 and b < 150) or (r > 120 and g > 70 and b < 100))
...
def rotate_and_relocate(src: Image.Image) -> tuple[Image.Image, dict[str, Any]]:
    ...
    # move the full 240×320 layer up by exactly UPWARD_SHIFT_PX pixels (the prior three-quarters fist-height move plus a 10px screen-right correction).
...
def labeled_contact(body: Image.Image, weapon: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    d.text((4, 20), f"battle_idle_right · grip={TARGET_GRIP[0]:.0f},{TARGET_GRIP[1]:.0f} · angle={TARGET_ANGLE_DEG:.1f}° up-right · handle@fist-center=(102,206); shift=+11px", fill=(24, 24, 24, 255))
    return out
...
def main() -> None:
    ...
    checks = {
        ...
        "handleContactSourcePixelPass": handle_pixel_pass(source, HANDLE_CONTACT_SOURCE),
        "handleContactTargetPixelPass": handle_pixel_pass(weapon, HANDLE_CONTACT_TARGET),
        "handleContactLocksToFistCenter": math.hypot(HANDLE_CONTACT_TARGET[0]-FIST_CENTER[0], HANDLE_CONTACT_TARGET[1]-FIST_CENTER[1]) <= 0.5,
    ...
    qa = {
        ...
        "generation": {"provider": None, "model": None, "credits": 0, "method": "deterministic reuse + integer right-up translation", "rawImageGeneration": False},
        ...
        "hand": {
            ...
            "handleContactToFistCenterDistancePx": math.hypot(HANDLE_CONTACT_TARGET[0]-FIST_CENTER[0], HANDLE_CONTACT_TARGET[1]-FIST_CENTER[1]),
        },
        "occlusion": {
    ...
    calibration = {
        "task": "T45",
        "revision": "hero-weapon-pilot-frame01-fist-center-lock-v7",
        "trialOnly": True,
        ...
        "fistCenterPx": {"x": FIST_CENTER[0], "y": FIST_CENTER[1]},
        "skinCoreCentroidPx": {"x": skin_centroid[0], "y": skin_centroid[1]},
        "handleContactPx": {"source": {"x": HANDLE_CONTACT_SOURCE[0], "y": HANDLE_CONTACT_SOURCE[1]}, "target": {"x": HANDLE_CONTACT_TARGET[0], "y": HANDLE_CONTACT_TARGET[1]}, "distanceToFistCenterPx": math.hypot(HANDLE_CONTACT_TARGET[0]-FIST_CENTER[0], HANDLE_CONTACT_TARGET[1]-FIST_CENTER[1])},
        "gripPoint": {"x": TARGET_GRIP[0], "y": TARGET_GRIP[1]},
        ...
        "sourceRevision": str((REV.parent / "right-up-placement-v6/manifest.json").relative_to(ROOT)),
        "supersedes": "hero-weapon-pilot-frame01-right-up-placement-v6",
    ...
    manifest = {
        ...
        "integrationGate": "not_handed_off",
        "runtimeRelease": False,
        "scope": "hero battle_idle_right first-frame only; deterministic screen-right placement correction; no runtime integration",
        "supersedes": str((REV.parent / "right-up-placement-v6/manifest.json").relative_to(ROOT)),
        ...
        "visualReviewNote": "v7 keeps the v6 character-right screen-left-fist body, -40° up-right axis, and prior placement, then shifts the whole sword 11px screen-right so the handle contact pixel locks to fist center (102,206)",
        "formalRuntimeTouched": False,
    }
    MANIFEST_PATH.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n")
    JOB_PATH.write_text(json.dumps({
        "task": "T45", "seq": "hero-weapon-pilot-frame01", "revision": "fist-center-lock-v7",
        ...
        "reviewReason": "Leo follow-up correction: put the sword handle into the fist center; keep body and angle unchanged",
        "correction": {"from": "v6 grip (110,185) / handle contact (91,206)", "to": "grip (121,185); screen-right shift=11px; handle contact locked to fist center (102,206); angle unchanged"},
        "method": "deterministic reuse of v6 transparent sword layer; integer screen-right shift only; no new generation; body frozen",
        "runtimeWrite": False,
    }, ensure_ascii=False, indent=2) + "\n")
    REQUEST_PATH.write_text("""# T45 hero weapon frame01 · fist-center lock correction v7
...
""")
    references = [
        ...
        {"path": str(SOURCE_LAYER.relative_to(ROOT)), "role": "v6 normalized transparent sword geometry", "allowed": "deterministic integer screen-right translation only; lock handle contact to fist center (102,206)", "forbidden": "redraw/rotation/shape change/new generation", "sha256": sha256(SOURCE_LAYER)},
    ]
    ...
    REFS_PATH.write_text(json.dumps({
        "task": "T45", "revision": "fist-center-lock-v7", "generationCredits": 0,
        "references": references,
        "correction": {"hand": "character_right_hand_screen_left_fist", "sourceGrip": SOURCE_GRIP, "targetGrip": TARGET_GRIP, "fistHeightPx": FIST_HEIGHT_PX, "upwardFraction": UPWARD_FRACTION, "translationPx": list(TRANSLATION_PX), "angleDeg": TARGET_ANGLE_DEG, "rotationDeltaDeg": ROTATION_DELTA_DEG, "fistCenterPx": FIST_CENTER, "handleContactSourcePx": HANDLE_CONTACT_SOURCE, "handleContactTargetPx": HANDLE_CONTACT_TARGET, "directionReference": "Leo requested handle contact pixel locked to fist center"},
    }, ensure_ascii=False, indent=2) + "\n")
    print(json.dumps({"revision": "fist-center-lock-v7", "checks": qa["checks"], "weaponSha256": sha256(OUT_LAYER), "gripPoint": TARGET_GRIP, "angleDeg": TARGET_ANGLE_DEG, "axisLengthPx": axis_len}, ensure_ascii=False))
