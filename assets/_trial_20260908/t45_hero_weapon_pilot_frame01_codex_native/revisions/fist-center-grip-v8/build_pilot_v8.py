...
The v4 normalized sword layer is reused without redraw. This revision nudges the blade-base
grip by the exact integer vector (+2,+1) so it lands at the frozen fist center (102,206);
the -40° axis and all body pixels are preserved.
...
REV = BASE / "revisions/fist-center-grip-v8"
...
OUT_LAYER = REV / "normalized/hero_sword_held_rightup_v8.png"
MASK_PATH = REV / "occlusion_masks/hero_idle_right_character_right_fist.png"
TRIP_PATH = REV / "composites_native/hero_idle_right_sword_triptych_v8.png"
TRIP_2X_PATH = REV / "composites_2x/hero_idle_right_sword_triptych_v8_2x.png"
CONTACT_PATH = REV / "contact/hero_idle_right_sword_pilot_v8.png"
CONTACT_2X_PATH = REV / "contact/hero_idle_right_sword_pilot_v8_2x.png"
QA_PATH = REV / "qa/pilot_v8.json"
CAL_PATH = REV / "calibration/frame01_idle_right_v8.json"
...
SOURCE_GRIP = (100.0, 205.0)  # v4 blade-base grip, character right hand; screen-left fist
FIST_HEIGHT_PX = 26  # tight frozen fist-mask bbox y=193..218 inclusive
UPWARD_FRACTION = 0.0
UPWARD_SHIFT_PX = round(FIST_HEIGHT_PX * UPWARD_FRACTION)  # this revision does not change the vertical placement band
RIGHT_SHIFT_PX = 2
VERTICAL_NUDGE_PX = 1
...
HANDLE_CONTACT_SOURCE = (100.0, 205.0)
...
def rotate_and_relocate(src: Image.Image) -> tuple[Image.Image, dict[str, Any]]:
    ...
    # move the full 240×320 layer by the exact integer vector (+2,+1) to lock the blade-base grip to the fist center.
...
def labeled_contact(body: Image.Image, weapon: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    d.text((4, 20), f"battle_idle_right · grip={TARGET_GRIP[0]:.0f},{TARGET_GRIP[1]:.0f} · angle={TARGET_ANGLE_DEG:.1f}° up-right · grip@fist-center=(102,206); shift=+2,+1px", fill=(24, 24, 24, 255))
    return out
...
def main() -> None:
    ...
    calibration = {
        "task": "T45",
        "revision": "hero-weapon-pilot-frame01-fist-center-grip-v8",
        "trialOnly": True,
        ...
        "supersedes": "hero-weapon-pilot-frame01-fist-center-lock-v7",
    ...
    manifest = {
        ...
        "supersedes": str((REV.parent / "fist-center-lock-v7/manifest.json").relative_to(ROOT)),
        ...
        "visualReviewNote": "v8 keeps the v4 character-right screen-left-fist body and -40° up-right axis, then translates the blade-base grip by (+2,+1)px so the grip locks to fist center (102,206)",
        "formalRuntimeTouched": False,
    }
    MANIFEST_PATH.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n")
    JOB_PATH.write_text(json.dumps({
        "task": "T45", "seq": "hero-weapon-pilot-frame01", "revision": "fist-center-grip-v8",
        ...
        "reviewReason": "Leo follow-up correction: put the sword grip into the fist center by pixel alignment; keep body and angle unchanged",
        "correction": {"from": "v4 grip (100,205) / grip contact (100,205)", "to": "grip (102,206); integer shift=(+2,+1); grip locked to fist center (102,206); angle unchanged"},
        "method": "deterministic reuse of v4 transparent sword layer; integer screen-right shift only; no new generation; body frozen",
        "runtimeWrite": False,
    }, ensure_ascii=False, indent=2) + "\n")
    REQUEST_PATH.write_text("""# T45 hero weapon frame01 · pixel-locked fist-center grip correction v8\n\nLeo 要求认真按像素对齐，把剑放到拳心里。\n\n本 v8 只做确定性平移：\n- 身体冻结：`assets/characters/hero/battle45/battle_idle_right.png` 字节不变；紧拳皮肤核心全像素质心为 `(101.6136,205.9091)`，拳心取整为 `(102,206)`。\n- 继承 v4 握点 `(100,205)` 与 `-40°` 向右上轴线，整层平移 `(+2,+1)px`，目标握点为 `(102,206)`；剑柄/护手接触点与拳心重合。\n- 复用 v4 透明剑层，不重新生图、不旋转；角色身体与剑角度保持不变。\n- 用户参考图仅用于方向/握点语义核对：`refs/user_fist_center_sword_reference.png`；不描摹其像素。\n- `layerOrder=front`；拳部遮挡只取原身体 alpha 的紧右拳 ROI，不绘制替代手。\n- 仍为 `candidate-only`，正式 runtime 未写入，等待 Leo 视觉门与 PM 规格门。\n""")
    references = [
        ...
        {"path": str(SOURCE_LAYER.relative_to(ROOT)), "role": "v4 normalized transparent sword geometry", "allowed": "deterministic integer screen-right translation only; lock handle contact to fist center (102,206)", "forbidden": "redraw/rotation/shape change/new generation", "sha256": sha256(SOURCE_LAYER)},
    ]
    ...
    REFS_PATH.write_text(json.dumps({
        "task": "T45", "revision": "fist-center-grip-v8", "generationCredits": 0,
    ...
    print(json.dumps({"revision": "fist-center-grip-v8", "checks": qa["checks"], "weaponSha256": sha256(OUT_LAYER), "gripPoint": TARGET_GRIP, "angleDeg": TARGET_ANGLE_DEG, "axisLengthPx": axis_len}, ensure_ascii=False))
