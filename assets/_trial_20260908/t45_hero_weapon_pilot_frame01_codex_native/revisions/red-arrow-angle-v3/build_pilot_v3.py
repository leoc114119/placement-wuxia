...
The existing transparent sword layer is reused without redraw.  This revision rotates the
existing sword from the superseded v2 60° down-right axis to the canonical -40° up-right angle
and keeps the grip on the character's own right hand (the screen-left fist in battle_idle_right).
...
REV = BASE / "revisions/red-arrow-angle-v3"
...
OUT_LAYER = REV / "normalized/hero_sword_held_rightup_v3.png"
MASK_PATH = REV / "occlusion_masks/hero_idle_right_character_right_fist.png"
TRIP_PATH = REV / "composites_native/hero_idle_right_sword_triptych_v3.png"
TRIP_2X_PATH = REV / "composites_2x/hero_idle_right_sword_triptych_v3_2x.png"
CONTACT_PATH = REV / "contact/hero_idle_right_sword_pilot_v3.png"
CONTACT_2X_PATH = REV / "contact/hero_idle_right_sword_pilot_v3_2x.png"
QA_PATH = REV / "qa/pilot_v3.json"
CAL_PATH = REV / "calibration/frame01_idle_right_v3.json"
...
def labeled_contact(body: Image.Image, weapon: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    d.text((4, 20), f"battle_idle_right · grip={TARGET_GRIP[0]:.0f},{TARGET_GRIP[1]:.0f} · angle={TARGET_ANGLE_DEG:.1f}° up-right", fill=(24, 24, 24, 255))
    return out
...
def main() -> None:
    ...
    calibration = {
        "task": "T45",
        "revision": "hero-weapon-pilot-frame01-red-arrow-angle-v3",
        "trialOnly": True,
        ...
        "sourceRevision": str((REV.parent / "right-hand-angle-v2/manifest.json").relative_to(ROOT)),
        "supersedes": "hero-weapon-pilot-frame01-v2",
    ...
    manifest = {
        ...
        "supersedes": str((REV.parent / "right-hand-angle-v2/manifest.json").relative_to(ROOT)),
        ...
        "visualReviewNote": "v1/v2 superseded: v2 used 60° down-right; v3 keeps character-right screen-left-fist grip and uses -40° up-right per Leo red-arrow reference",
        "formalRuntimeTouched": False,
    }
    MANIFEST_PATH.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n")
    JOB_PATH.write_text(json.dumps({
        "task": "T45", "seq": "hero-weapon-pilot-frame01", "revision": "red-arrow-angle-v3",
...
- `layerOrder=front`；拳部遮挡只取原身体 alpha 的紧右拳 ROI，不绘制替代手。\n- 仍为 `candidate-only`，正式 runtime 未写入，等待 Leo 视觉门与 PM 规格门。\n""")
    ...
    REFS_PATH.write_text(json.dumps({
        "task": "T45", "revision": "red-arrow-angle-v3", "generationCredits": 0,
        "references": references,
        "correction": {"hand": "character_right_hand_screen_left_fist", "angleDeg": TARGET_ANGLE_DEG, "rotationDeltaDeg": ROTATION_DELTA_DEG, "directionReference": "Leo red arrow points up-right from fist center"},
    }, ensure_ascii=False, indent=2) + "\n")
    print(json.dumps({"revision": "red-arrow-angle-v3", "checks": qa["checks"], "weaponSha256": sha256(OUT_LAYER), "gripPoint": TARGET_GRIP, "angleDeg": TARGET_ANGLE_DEG, "axisLengthPx": axis_len}, ensure_ascii=False))
