#!/usr/bin/env python3
"""Build T45 hero weapon frame03 walk_right_2 from the accepted frame02 layer.

The accepted horizontal sword is translated to the next frame's character-right
fist center. This is deterministic Pillow compositing only: no generation,
body edits, rotation, resampling, or runtime writes.
"""
...
SOURCE_REV = BASE / "revisions/frame02-walk-right-1-horizontal-angle-v2"
REV = BASE / "revisions/frame03-walk-right-2-horizontal-v1"
BODY = ROOT / "assets/characters/hero/battle45/walk_right_2.png"
SOURCE = SOURCE_REV / "normalized/hero_sword_held_horizontal_walk_right_1_v2.png"
RAW = REV / "raw/hero_sword_held_horizontal_source_v2.png"
OUT = REV / "normalized/hero_sword_held_horizontal_walk_right_2_v1.png"
MASK = REV / "occlusion_masks/hero_walk_right_2_character_right_fist.png"
TRIP = REV / "composites_native/hero_walk_right_2_sword_triptych.png"
TRIP2 = REV / "composites_2x/hero_walk_right_2_sword_triptych_2x.png"
CONTACT = REV / "contact/hero_walk_right_2_sword_pilot.png"
CONTACT2 = REV / "contact/hero_walk_right_2_sword_pilot_2x.png"
ZOOM = REV / "contact/walk_right_2_fist_center_zoom.png"
QA = REV / "qa/pilot_walk_right_2_v1.json"
CAL = REV / "calibration/frame03_walk_right_2_v1.json"
...
SOURCE_HANDLE = (83.573696, 206.5878)
...
def contact(body: Image.Image, weapon: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    d.text((4, 22), "walk_right_2 · grip≈116.49,203.59 · angle=0° horizontal · translate=(+18,-3)", fill=(20, 20, 20, 255))
    return out
...
def main() -> None:
    ...
    axis_pts = alpha_points(weapon); grip = (HANDLE_CENTER[0] + AXIS_LENGTH / 2, HANDLE_CENTER[1])
    axis = max(x for x, _ in axis_pts) - min(x for x, _ in axis_pts)
    checks = {
        ...
        "weaponAngleHorizontalPass": ANGLE == 0.0,
    ...
    qa = {
        "task": "T45", "seq": "hero-weapon-pilot-frame03-walk-right-2", "revision": "frame03-walk-right-2-horizontal-v1",
        ...
        "generation": {"provider": None, "model": None, "credits": 0, "method": "deterministic integer translation of accepted frame02 horizontal sword layer", "rawImageGeneration": False},
        "body": {"path": body_rel, "sha256": sha(BODY), "metrics": bm},
        "weapon": {"sourceLayer": source_rel, "sourceLayerSha256": sha(SOURCE), "normalizedPath": out_rel, "normalizedSha256": sha(OUT), "metrics": wm, "axisLengthPx": axis, "angleDeg": ANGLE, "translationPx": [TX, TY]},
        ...
        "transform": {"sourceRevision": "frame02-walk-right-1-horizontal-angle-v2", "translationPx": [TX, TY], "rotationDeg": 0.0, "resampling": "none; integer RGBA translation"},
    ...
    CAL.write_text(json.dumps({"task": "T45", "revision": "frame03-walk-right-2-horizontal-v1", "trialOnly": True, "frame": "walk_right_2.png", "visualReview": "pending_Leo", "bodyPath": body_rel, "bodySha256": sha(BODY), "weaponPath": out_rel, "weaponSha256": sha(OUT), "handSemantic": "character_right_hand", "screenProjection": "screen-left fist", "fistCenterPx": {"x": 102, "y": 204}, "skinCoreCentroidPx": {"x": sx, "y": sy, "pixels": len(comp)}, "handleCenterPx": {"source": list(SOURCE_HANDLE), "target": list(HANDLE_CENTER), "distanceToFistCenterPx": math.hypot(HANDLE_CENTER[0] - FIST_CENTER[0], HANDLE_CENTER[1] - FIST_CENTER[1])}, "gripPointPx": {"x": grip[0], "y": grip[1]}, "angleDeg": ANGLE, "angleDefinition": "blade axis from grip toward tip; 0°=screen-right, +Y down, clockwise-positive", "layerOrder": "front", "occlusionRef": {"maskPath": mask_rel, "maskSha256": sha(MASK)}, "sourceRevision": "frame02-walk-right-1-horizontal-angle-v2", "translationPx": [TX, TY], "status": "candidate_only", "runtimeRelease": False}, ensure_ascii=False, indent=2) + "\n")
    MANIFEST.write_text(json.dumps({"task": "T45", "revision": "frame03-walk-right-2-horizontal-v1", "artifactStage": "candidate", "visualReview": "pending_Leo", "specGate": "pending_pm_scan", "integrationGate": "not_handed_off", "runtimeRelease": False, "scope": "hero battle walk_right_2 frame03; accepted horizontal sword translated to next fist center", "sourceBody": body_rel, "sourceBodySha256": sha(BODY), "sourceWeaponLayer": source_rel, "sourceWeaponLayerSha256": sha(SOURCE), "weaponNormalized": out_rel, "weaponSha256": sha(OUT), "handSemantic": "character_right_hand_screen_left_fist", "fistCenterPx": {"x": 102, "y": 204}, "handleCenterPx": {"x": HANDLE_CENTER[0], "y": HANDLE_CENTER[1]}, "gripPointPx": {"x": grip[0], "y": grip[1]}, "angleDeg": ANGLE, "layerOrder": "front", "translationPx": [TX, TY], "calibration": str(CAL.relative_to(ROOT)), "qa": str(QA.relative_to(ROOT)), "contact": str(CONTACT.relative_to(ROOT)), "fistCenterZoom": str(ZOOM.relative_to(ROOT)), "formalRuntimeTouched": False, "supersedes": source_rel, "status": "candidate_only", "visualReviewEvidence": None, "note": "Next frame built by translating accepted frame02 horizontal layer; no rotation, generation, body edit, or runtime write"}, ensure_ascii=False, indent=2) + "\n")
    JOB.write_text(json.dumps({"task": "T45", "seq": "hero-weapon-pilot-frame03-walk-right-2", "revision": "frame03-walk-right-2-horizontal-v1", "artifactStage": "candidate", "visualReview": "pending_Leo", "specGate": "pending_pm_scan", "integrationGate": "not_handed_off", "runtimeRelease": False, "status": "candidate_only", "sourceRevision": "frame02-walk-right-1-horizontal-angle-v2", "correction": "Translate the accepted horizontal sword layer (+18,-3) to walk_right_2 character-right fist center (102,204); preserve angle 0° and sword geometry", "method": "deterministic integer RGBA translation; no generation, rotation, body edit, or runtime write", "runtimeWrite": False, "nextGate": "Leo visual review; PM gate deferred until all action frames are determined"}, ensure_ascii=False, indent=2) + "\n")
    REQ.write_text(f"""# T45 hero weapon frame03 walk_right_2 · horizontal sword v1\n\nFrame02 horizontal sword was visually accepted. Build the next frame by translating the accepted layer only.\n\n- Frozen body: `{body_rel}`.\n- Frozen accepted source layer: `{source_rel}`.\n- Character-right hand is the screen-left fist; measured fist center is `(102,204)`.\n- Translate the accepted layer by `(+18,-3)` from frame02. Preserve horizontal `0°`, layer order `front`, sword geometry and blur level.\n- No rotation, redraw, generation, resampling, body edit, or runtime write. Credits=0.\n- Candidate-only; Leo visual review pending. PM second gate remains deferred until all action frames are determined.\n""")
    REFS.write_text(json.dumps({"task": "T45", "revision": "frame03-walk-right-2-horizontal-v1", "generationCredits": 0, "references": [{"path": body_rel, "role": "frozen hero walk_right_2 body and hand geometry", "allowed": "hand center and precise fist occlusion mask only", "forbidden": "body redraw/pose/identity changes", "sha256": sha(BODY)}, {"path": source_rel, "role": "Leo-accepted frame02 horizontal sword layer", "allowed": "integer translation only", "forbidden": "rotation, redraw, resampling, generation", "sha256": sha(SOURCE)}], "transform": {"sourceRevision": "frame02-walk-right-1-horizontal-angle-v2", "sourceHandleCenterPx": list(SOURCE_HANDLE), "targetHandleCenterPx": list(HANDLE_CENTER), "sourceFistCenterPx": list(SOURCE_FIST), "targetFistCenterPx": list(FIST_CENTER), "translationPx": [TX, TY], "angleDeg": ANGLE}}, ensure_ascii=False, indent=2) + "\n")
    print(json.dumps({"revision": "frame03-walk-right-2-horizontal-v1", "checks": checks, "skinCoreCentroid": [sx, sy, len(comp)], "fistCenter": FIST_CENTER, "handleCenter": HANDLE_CENTER, "weaponSha256": sha(OUT)}, ensure_ascii=False))
