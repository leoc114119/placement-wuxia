...
BODY_SOURCE=BODY
BODY=REV/'body/battle_idle_rightdown_fist_out_v7.png'
SOURCE=ROOT/'assets/_trial_20260908/t45_hero_weapon_pilot_frame01_codex_native/normalized/hero_sword_held_rightdown.png'
RAW=REV/'raw/hero_sword_source_rightdown_v1.png'; OUT=REV/'normalized/hero_sword_held_idle_rightdown_character_right_hand_v7.png'; MASK=REV/'occlusion_masks/hero_idle_rightdown_screen_left_fist_v7.png'; TRIP=REV/'composites_native/hero_idle_rightdown_sword_character_right_hand_triptych_v7.png'; TRIP2=REV/'composites_2x/hero_idle_rightdown_sword_character_right_hand_triptych_v7_2x.png'; CONTACT=REV/'contact/hero_idle_rightdown_sword_character_right_hand_v7.png'; ZOOM=REV/'contact/idle_rightdown_character_right_fist_center_zoom_v7.png'; QA=REV/'qa/pilot_idle_rightdown_character_right_hand_v7.json'; CAL=REV/'calibration/frame06_idle_rightdown_character_right_hand_v7.json'
W,H=240,320
TX,TY=43,-13
...
def trip(body,weapon,mask):
 # Weapon-front mode: the sword is in front of the body, then the complete original-alpha fist cutout is restored above it.
...
def main():
 ...
 ct=Image.new('RGBA',(W*3,H+42),(236,236,236,255));ct.alpha_composite(tri,(0,42));d=ImageDraw.Draw(ct);d.text((4,4),'WEAPON FRONT + complete fist cutout top · character RIGHT hand (screen-left fist)',fill=(20,20,20,255));d.text((4,22),f'idle_rightdown · handle≈({TARGET_HANDLE[0]:.2f},{TARGET_HANDLE[1]:.2f}) · mirrored+grip-point-lock rightdown v7 + ({TX},{TY}) · angle=-75.000°',fill=(20,20,20,255));ct.save(CONTACT);ct.resize((ct.width*2,ct.height*2),Image.Resampling.NEAREST).save(REV/'contact/hero_idle_rightdown_sword_character_right_hand_v7_2x.png')
 ...
 checks={'bodyCanvasPass':bm['size']==[W,H],'bodyAlphaPass':bm['alphaExtrema']==[0,255],'bodyBorderTransparent':bm['borderNonzero']==0,'weaponCanvasPass':wm['size']==[W,H],'weaponRealAlphaPass':wm['alphaExtrema']==[0,255],'weaponSingleConnectedSilhouette':comps(weapon)==1,'weaponLengthBandPass':110<=axis<=140,'weaponAnglePass':abs(ANGLE+75.0)<1e-3,'handleCenterPass':abs(TARGET_HANDLE[0]-FIST[0])<1.0 and abs(TARGET_HANDLE[1]-FIST[1])<1.0,'weaponFrontCompleteFistCutoutTop':True,'runtimeUntouched':True,'generationCreditsZero':True}
 qa={'task':'T45','seq':'hero-weapon-pilot-frame06-idle-rightdown-character-right-hand','revision':'frame06-idle-rightdown-sword-character-right-hand-v7-complete-fist-top','artifactStage':'candidate','visualReview':'pending_Leo','specGate':'pending_pm_scan','integrationGate':'not_handed_off','runtimeRelease':False,'generation':{'credits':0,'method':'deterministic horizontal mirror, pivot rotation and integer translation of the existing rightdown sword source','rawImageGeneration':False},'body':{'path':str(BODY.relative_to(ROOT)),'sha256':sha(BODY),'metrics':bm,'sourcePath':str(BODY_SOURCE.relative_to(ROOT)),'sourceSha256':sha(BODY_SOURCE),'correction':'screen-left projected fist (character right hand) local +15° outward rotation around wrist; body candidate only'},'weapon':{'sourceLayer':str(SOURCE.relative_to(ROOT)),'sourceLayerSha256':sha(SOURCE),'normalizedPath':str(OUT.relative_to(ROOT)),'normalizedSha256':sha(OUT),'metrics':wm,'transform':'horizontal mirror then -151.377° pivot rotation around mirrored grip; grip point (mirrored source 74,185) translated to fist center pixel (102,195); 15° toward face final axis','translationPx':[TX,TY],'angleDeg':ANGLE,'handleCenterSourcePx':list(SOURCE_HANDLE),'handleCenterTargetPx':list(TARGET_HANDLE)},'hand':{'semantic':'character_right_hand','screenProjection':'screen-left fist in idle_rightdown','fistCenterPx':list(FIST),'fistPixels':len(fist),'gripPointPx':list(GRIP)},'occlusion':{'layerOrder':'weapon_front','maskPolicy':'complete_fist_cutout_top','maskPath':str(MASK.relative_to(ROOT)),'maskSha256':sha(MASK),'weaponInFistBefore':before,'weaponInFistAfter':after},'composites':{'native':str(TRIP.relative_to(ROOT)),'twoX':str(TRIP2.relative_to(ROOT)),'contact':str(CONTACT.relative_to(ROOT)),'fistZoom':str(ZOOM.relative_to(ROOT))},'checks':checks|{'allMachineChecksPass':all(checks.values())},'note':'Latest Leo direction: sword layer is in front, and only the complete character-right fist cutout is restored above it. The screen-left projected fist is the character-right hand in this rightdown pose; it is rotated +15° outward. The marked lower-handle grip point (mirrored source 74,185) is translated to fist center pixel (102,195); the sword axis remains 15° toward the face from vertical.'}
 QA.write_text(json.dumps(qa,ensure_ascii=False,indent=2)+'\n');CAL.write_text(json.dumps({'task':'T45','revision':'frame06-idle-rightdown-sword-character-right-hand-v7-complete-fist-top','bodyPath':str(BODY.relative_to(ROOT)),'bodySourcePath':str(BODY_SOURCE.relative_to(ROOT)),'bodySourceSha256':sha(BODY_SOURCE),'bodySha256':sha(BODY),'weaponPath':str(OUT.relative_to(ROOT)),'weaponSha256':sha(OUT),'handSemantic':'character_right_hand','screenProjection':'screen-left fist in idle_rightdown','fistCenterPx':list(FIST),'handleCenterSourcePx':list(SOURCE_HANDLE),'handleCenterTargetPx':list(TARGET_HANDLE),'translationPx':[TX,TY],'angleDeg':ANGLE,'layerOrder':'weapon_front','maskPolicy':'complete_fist_cutout_top','status':'candidate_only','runtimeRelease':False},ensure_ascii=False,indent=2)+'\n');print(json.dumps({'checks':checks,'allMachineChecksPass':all(checks.values()),'weaponBBox':wm['bboxT32'],'fist':FIST,'handleTarget':TARGET_HANDLE},ensure_ascii=False))
main()
