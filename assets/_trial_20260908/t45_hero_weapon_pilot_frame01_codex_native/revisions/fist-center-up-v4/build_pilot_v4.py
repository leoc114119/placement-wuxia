...
The v3 normalized sword layer is reused without redraw.  This revision translates the
whole layer upward by 3px so its handle/grip center aligns with the frozen screen-left fist
center in battle_idle_right.  The -40° up-right axis and all body pixels remain frozen.
...
REV = BASE / "revisions/fist-center-up-v4"
BODY_PATH = ROOT / "assets/characters/hero/battle45/battle_idle_right.png"
SOURCE_LAYER = BASE / "revisions/red-arrow-angle-v3/normalized/hero_sword_held_rightup_v3.png"
SOURCE_RAW = BASE / "revisions/red-arrow-angle-v3/raw/hero_sword_held_transparent_source_attempt1.png"
OUT_LAYER = REV / "normalized/hero_sword_held_rightup_v4.png"
MASK_PATH = REV / "occlusion_masks/hero_idle_right_character_right_fist.png"
TRIP_PATH = REV / "composites_native/hero_idle_right_sword_triptych_v4.png"
TRIP_2X_PATH = REV / "composites_2x/hero_idle_right_sword_triptych_v4_2x.png"
CONTACT_PATH = REV / "contact/hero_idle_right_sword_pilot_v4.png"
CONTACT_2X_PATH = REV / "contact/hero_idle_right_sword_pilot_v4_2x.png"
QA_PATH = REV / "qa/pilot_v4.json"
CAL_PATH = REV / "calibration/frame01_idle_right_v4.json"
...
SOURCE_GRIP = (100.0, 208.0)  # v3 grip, character right hand; screen-left fist
TARGET_GRIP = (100.0, 205.0)  # fist-center y from frozen body alpha centroid ≈204.77
...
def rotate_and_relocate(src: Image.Image) -> tuple[Image.Image, dict[str, Any]]:
    # v4 is deliberately translation-only: preserve every v3 sword pixel and
    # move the full 240×320 layer up by exactly 3px to the measured fist center.
...
def labeled_contact(body: Image.Image, weapon: Image.Image, mask: Image.Image) -> Image.Image:
    ...
    d.text((4, 20), f"battle_idle_right · grip={TARGET_GRIP[0]:.0f},{TARGET_GRIP[1]:.0f} · angle={TARGET_ANGLE_DEG:.1f}° up-right · y-shift=-3px", fill=(24, 24, 24, 255))
    return out
...
def main() -> None:
    ...
    calibration = {
        "task": "T45",
        "revision": "hero-weapon-pilot-frame01-fist-center-up-v4",
        "trialOnly": True,
        ...
        "sourceRevision": str((REV.parent / "red-arrow-angle-v3/manifest.json").relative_to(ROOT)),
        "supersedes": "hero-weapon-pilot-frame01-red-arrow-angle-v3",
    ...
    manifest = {
        ...
        "supersedes": str((REV.parent / "red-arrow-angle-v3/manifest.json").relative_to(ROOT)),
        ...
        "visualReviewNote": "v4 keeps v3 character-right screen-left-fist grip and -40° up-right axis, translating the sword up 3px to center the handle on the fist",
        "formalRuntimeTouched": False,
    }
    MANIFEST_PATH.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n")
    JOB_PATH.write_text(json.dumps({
        "task": "T45", "seq": "hero-weapon-pilot-frame01", "revision": "fist-center-up-v4",
    ...
    references = [
        ...
        {"path": str(SOURCE_LAYER.relative_to(ROOT)), "role": "v3 normalized transparent sword geometry", "allowed": "deterministic integer translation upward only", "forbidden": "redraw/rotation/shape change/new generation", "sha256": sha256(SOURCE_LAYER)},
    ]
    ...
    REFS_PATH.write_text(json.dumps({
        "task": "T45", "revision": "fist-center-up-v4", "generationCredits": 0,
        "references": references,
        "correction": {"hand": "character_right_hand_screen_left_fist", "sourceGrip": SOURCE_GRIP, "targetGrip": TARGET_GRIP, "translationPx": list(TRANSLATION_PX), "angleDeg": TARGET_ANGLE_DEG, "rotationDeltaDeg": ROTATION_DELTA_DEG, "directionReference": "Leo red arrow points up-right from fist center"},
    }, ensure_ascii=False, indent=2) + "\n")
    print(json.dumps({"revision": "fist-center-up-v4", "checks": qa["checks"], "weaponSha256": sha256(OUT_LAYER), "gripPoint": TARGET_GRIP, "angleDeg": TARGET_ANGLE_DEG, "axisLengthPx": axis_len}, ensure_ascii=False))
