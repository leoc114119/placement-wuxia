#!/usr/bin/env python3
"""T45 批 1 补丁 6 · cast_down_2 劈停画面左下重出（1 张，Leo 逐字任务书·画面坐标版）。
与 patch_cast5 的差异（任务书改口径）：
  ①prompt 换画面坐标版（全篇仅画面上下左右，禁身体左/右方位词）——上一版败因=prompt 混用坐标系
  ②stop 点新增拳头左右落点程序化实测：右格主体像素质心 x 相对右半区中点
    （raw 坐标按实际宽度换算 640 宽口径=480 / 归一坐标=120）的左右判定
其余口径同 patch5：640×320 双格表（左格原点粘贴）、链式锚=cast_down_1 成品、3:2（拒→9:16 一次并登记）、
门清单仅 check_left_v2 T=30（FAIL→同命令重跑 1 次→再犯登记停上报）、高度比只实测记录、mxai 抠图重试不超 1 次。
用法:
  python3 run_patch6.py gen     # 生图+check_left_v2+测量
  python3 run_patch6.py finish  # 抠图+归一+入库（gen 过门后）
"""
import json
import os
import subprocess
...
PC = os.path.join(B1, "patch_cast6")
...
RAW = os.path.join(PC, f"{NAME}_raw.png")        # 任务书步骤 4：--out patch_cast6/cast_down_2_raw.png
...
def fist_side(img, tag):
    """任务书 stop 点实测：右半区主体像素质心 x 相对右半区中点的左右判定。
    返回 dict（raw 实测 + 换算 640 宽口径便于对照任务书 480）。"""
    W, H = img.size
    half = W // 2
    R = cc_largest(img.crop((half, 0, W, H)))
    if R is None:
        print(f"[fist_side:{tag}] 右半区无主体")
        return None
    cx_raw = half + R[2]                      # 质心 x（raw 全图坐标）
    mid_raw = half + (W - half) / 2           # 右半区中点（raw）
    cx640 = cx_raw * 640.0 / W                # 换算 640 宽口径
    mid640 = mid_raw * 640.0 / W              # =480（640 宽口径）
    env = (R[1][0] + half, R[1][1], R[1][2] + half, R[1][3])  # 包络（raw 全图坐标）
    side = "画面左半" if cx_raw < mid_raw else "画面右半"
    out = {"centroid_x_raw": round(cx_raw, 1), "right_mid_raw": round(mid_raw, 1),
           "centroid_x_640": round(cx640, 1), "right_mid_640": round(mid640, 1),
           "env_raw": list(env), "cc": R[0], "side": side}
    print(f"[fist_side:{tag}] 主体质心 x={cx_raw:.1f}（640口径 {cx640:.1f}） vs 右半区中点 "
          f"{mid_raw:.1f}（640口径 {mid640:.1f}） → 主体偏{side}；包络 x[{env[0]},{env[2]}] cc={R[0]}")
    return out


# ---------- 记录 ----------
def log_read():
    p = os.path.join(PC, "measure_log_patch6.json")
    if os.path.exists(p):
...
def credits_append(name, note):
    ...
    arr.append({"name": name, "model": "gpt-image-2", "cost": 2,
                "ts": "2026-09-04 T45-batch1-patch6", "note": note})
    with open(p, "w", encoding="utf-8") as f:
...
def stage_gen():
    ...
    credits_append(NAME, f"劈停画面左下重出（画面坐标版 prompt）：640x320 双格表双参照 img2img "
                         f"aspect={aspect_note}，左格基准=cast_down_1.png（链式锁手别）；prompt 任务书逐字")
    ...
    # 测量（门=任务书清单仅 check_left；高度比/宽高比只实测记录不拦截；stop 点=拳头左右落点实测）
    ...
    print(f"[gates] left_env={L[1] if L else None} h={lh} | right w={rw} h={rh} "
    ...
    rec = {"frame": NAME, "raw_size": [W, H], "aspect": aspect_note, "check_left": round(mean, 3),
           ...
           "right_env_raw": fs["env_raw"] if fs else None,
           "right_w": rw, "right_h": rh, "right_cc": R[0] if R else 0,
...
def stage_finish():
    ...
    canvas.save(final)
    # 复核（含归一坐标 120 左右判定）
    fimg = Image.open(final)
    fa = fimg.getchannel("A")
    fb = fa.point(lambda v: 255 if v > 0 else 0).getbbox()
    fdata = list(fimg.getdata())
    fw = 240; fsw = fsx = 0
    ...
    fside = "画面左半" if fcent < 120 else "画面右半"
    print(f"[final] {final} bbox={fb} env={fb[2]-fb[0]}x{fb[3]-fb[1]} bottom={fb[3]} "
          f"centroid_x={fcent:.2f} aspect={fasp:.3f}")
    print(f"[fist_side:norm] 归一坐标主体质心 x={fcent:.2f} vs 中点 120 → 主体偏{fside}"
          f"（注：归一管线按质心 x=120 粘贴，此值≈120 属管线锁定事实，判别以 raw 实测为准）")
    rec = log_read().get(NAME)
    rec["result"] = "OK"
    rec["final_bbox"] = list(fb)
    rec["final_env"] = [fb[2] - fb[0], fb[3] - fb[1]]
    rec["final_centroid_x"] = round(fcent, 2)
    rec["fist_side_norm"] = {"centroid_x": round(fcent, 2), "mid": 120, "side": fside,
                             "note": "归一管线质心锁定 x=120，判别以 raw 实测为准"}
