#!/usr/bin/env python3
"""T45 批 1 补丁 · 施放帧修正（cast_down_1 左扫 / cast_down_2 右扫）单帧驱动。
规格（任务书 art-pipeline T45 批 1 补丁）：施放=挥剑连击（左扫→右扫→回位）；
身体帧禁任何武器；cast_down_3 不生成（主会话复用定锚帧）。
流程同批 1 run_frame.py：3:2 双格表（左格=定锚帧）+ check_left_v2 T=30 内容对齐
+ 高度比 ±10% 门 + mxai 抠图（右半区）+ 归一 256/y300/x120。
中间件全部落 patch_cast/；prompt 逐字来自任务书（共用骨架+姿势词条），禁自拟。
用法:
  python3 run_patch.py gen <帧名>     # 建表+配方+生图+check_left_v2+门测量
  python3 run_patch.py finish <帧名>  # 抠图+归一+入库（gen GATED 后）
"""
import json
import os
import subprocess
...
FRAMES = {
    "cast_down_1": {"pose": "施放挥砍动作·向左横扫：上身微向左转，右臂伸直向身体左侧横扫展开、五指合拢握拳（挥击轨迹扫向左侧），左手向右侧自然展开平衡，双腿微张发力。"},
    "cast_down_2": {"pose": "施放挥砍动作·向右横扫：上身微向右转，右臂伸直向身体右侧横扫展开、五指合拢握拳（挥击轨迹扫向右侧），左手自然垂放身侧，双腿微张发力。"},
}
...
# ---------- 记录 ----------
def log_read():
    p = os.path.join(PC, "measure_log_patch.json")
    if os.path.exists(p):
...
def credits_append(name, note):
    ...
    arr.append({"name": name, "model": "gpt-image-2", "cost": 2,
                "ts": "2026-09-04 T45-batch1-patch", "note": note})
    with open(p, "w", encoding="utf-8") as f:
...
def measure_gates(name, raw, mean, ok, rerun=None):
    ...
    gok = H_BAND[0] <= hr <= H_BAND[1]   # 任务书：门=高度比 ±10%（挥砍轮廓变化，免宽高比门，宽高比仅实测记录）
...
def stage_gen(name):
    ...
    credits_append(name, "施放帧修正（挥砍规格）：锚定表双参照 img2img aspect=3:2，左格=battle_idle_down.png")
