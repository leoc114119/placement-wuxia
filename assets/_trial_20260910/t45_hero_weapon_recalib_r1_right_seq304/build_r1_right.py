...
SCALES = {
    # seq=302 PM2 ruling: clamp the whole seq278 group so its widest source
    # crop (299px) fits the 238px content limit without any per-frame crop.
    "seq278_walk_jump": min(256 / 312, 238 / 299),
    "seq279_atk": 256 / 214,
}
...
def threshold_bbox(image: Image.Image, threshold: int = THRESHOLD) -> tuple[int, int, int, int] | None:
    alpha = image.getchannel("A")
    points = [
        (x, y)
        for y in range(image.height)
...
def normalize(cleaned: Image.Image, scale: float) -> tuple[Image.Image, dict]:
    ...
    y = FEET - height
    def compose(x_position: int) -> Image.Image:
        canvas = Image.new("RGBA", (W, H), (0, 0, 0, 0))
        canvas.alpha_composite(resized, (x_position, y))
        return canvas

    output = compose(x)
    # A one-pixel integer translation is allowed for centering and is not a
    # scale/crop operation. Prefer a border-clean placement when it still
    # keeps the required alpha>32 centroid within x=120±1.
    initial_x = x
    for candidate_x in (x - 1, x + 1):
        if candidate_x < 0 or candidate_x + width > W:
            continue
        candidate = compose(candidate_x)
        edge = border_nonzero(candidate)
        if edge["total"] == 0 and abs(weighted_centroid_x(candidate) - CX) <= 1:
            output = candidate
            x = candidate_x
            break
    ...
    return output, {
        ...
        "placedAt": [x, y],
        "placementAdjustedForBorder": x != initial_x,
...
def main() -> None:
    ...
    qa = {
        ...
        "groupCoefficientClamp": {
            "seq278_walk_jump": {
                "anchorScale": 256 / 312,
                "maxSourceCropWidth": 299,
                "contentWidthLimit": 238,
                "fixedScale": SCALES["seq278_walk_jump"],
                "formula": "min(anchorScale, contentWidthLimit / maxSourceCropWidth)",
            },
            "seq279_atk": {
                "anchorScale": 256 / 214,
                "fixedScale": SCALES["seq279_atk"],
                "formula": "anchorScale; no width clamp needed",
            },
        },
        "frames": {},
        "shaDistinct": False,
    ...
    for frame_id, source in SOURCES.items():
        ...
        source_bbox_tuple = threshold_bbox(cleaned)
        if source_bbox_tuple is None:
            raise ValueError(f"empty alpha>{THRESHOLD} source: {frame_id}")
        source_bbox = list(source_bbox_tuple)
        ...
        expected_scaled_size = [
            round(source_width * SCALES[GROUP[frame_id]]),
            round(source_height * SCALES[GROUP[frame_id]]),
        ]
        observed_size = [m["visualWidthAlphaGt32"], m["visualHeightAlphaGt32"]]
        checks = {
            ...
            "noCropWidthWithin1": abs(observed_size[0] - expected_scaled_size[0]) <= 1,
            "noCropHeightWithin1": abs(observed_size[1] - expected_scaled_size[1]) <= 1,
        }
        qa["frames"][frame_id] = {
            "source": str(source),
            ...
            "expectedScaledSize": expected_scaled_size,
            "observedAlphaGt32VisualSize": observed_size,
    ...
    qa["stopReason"] = (
        "after seq=302 group clamp, jump_right_3 has no integer placement that simultaneously "
        "keeps all four borders transparent and alpha>32 centroid x=120±1; x=6 centers but clips, "
        "x=1 preserves the frame but centroid is x≈115.166; do not crop or per-frame scale"
    ...
    contact_path = ROOT / "contact" / "right_r1_seq304_with_benchmark.png"
    draw_contact(contact_items, contact_path)
    qa["contact"] = str(contact_path)
    (ROOT / "qa" / "r1_right_seq304.json").write_text(json.dumps(qa, ensure_ascii=False, indent=2) + "\n")
    ...
    manifest = {
        ...
        "contact": "contact/right_r1_seq304_with_benchmark.png",
    ...
    print(json.dumps({"seq": "304", "frames": 10, "shaDistinct": qa["shaDistinct"], "allHardGatesPass": qa["allHardGatesPass"], "contact": str(contact_path)}, ensure_ascii=False))
