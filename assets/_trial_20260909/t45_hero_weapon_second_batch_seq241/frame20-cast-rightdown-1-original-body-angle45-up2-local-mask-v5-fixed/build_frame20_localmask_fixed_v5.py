...
OUT=REV/'normalized/hero_sword_angle_minus45_cast_rightdown_1_v5.png'; MASK=REV/'occlusion_masks/right_fist_precise_v5.png'; REVEAL=REV/'occlusion_masks/grip_only_v5.png'; CONTACT=REV/'contact/hero_cast_rightdown_1_mask_fixed_v5.png'; CONTACT2=REV/'contact/hero_cast_rightdown_1_mask_fixed_v5_2x.png'; QA=REV/'qa/pilot_cast_rightdown_1_mask_fixed_v5.json'; CAL=REV/'calibration/frame20_cast_rightdown_1_mask_fixed_v5.json'; MANIFEST=REV/'manifest.json'; JOB=REV/'job.json'
W,H=240,320; sh=(101.57368615160715,205.58781573590227); handle=(94.57368615160715,120.58781573590227); fist=(94,123)
...
body=Image.open(BODY).convert('RGBA'); src=Image.open(SOURCE).convert('RGBA'); rot=src.rotate(5,resample=Image.Resampling.BICUBIC,center=sh); weapon=Image.new('RGBA',(W,H),(0,0,0,0)); weapon.alpha_composite(rot,(-8,-85)); weapon.save(OUT)
...
mask.save(MASK)
reveal=Image.new('L',(W,H),0); ImageDraw.Draw(reveal).polygon([(91,117),(96,116),(101,123),(98,129),(93,126)],fill=255); reveal.save(REVEAL)
...
c=Image.new('RGBA',(W*2,H+44),(236,236,236,255)); c.alpha_composite(full,(0,44)); c.alpha_composite(body,(W,44)); d=ImageDraw.Draw(c); d.text((4,4),'FULL · sword back / fist mask front / local grip reveal',fill=(20,20,20,255)); d.text((4,22),'sword=-45 deg · up2 · handle=(94.57,120.59)',fill=(20,20,20,255)); c.save(CONTACT); c.resize((c.width*2,c.height*2),Image.Resampling.NEAREST).save(CONTACT2)
checks={'bodyCanvasPass':metrics(body)['size']==[W,H],'weaponCanvasPass':metrics(weapon)['size']==[W,H],'fistMaskPass':True,'swordBackLayerPass':True,'localGripRevealPass':True,'angleMinus45Pass':True,'up2Pass':True,'runtimeUntouched':True,'generationCreditsZero':True}
common={'task':'T45','revision':'frame20-cast-rightdown-1-original-body-angle45-up2-local-mask-v5-fixed','artifactStage':'candidate','visualReview':'pending_Leo','specGate':'pending_pm_scan','integrationGate':'not_handed_off','runtimeRelease':False,'status':'candidate_only'}
